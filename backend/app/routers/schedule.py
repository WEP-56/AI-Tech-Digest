...
from app.schemas import ScheduleConfigResponse, ScheduleConfigUpdate, JobActionResponse
...
def trigger_fetch(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    try:
        ...
        db.commit()
        return JobActionResponse(success=True, message=f"采集完成: 成功 {result['success']}/{result['total']}, 新增 {result['new_items']} 条", job_id=job.id)
...
def trigger_generate(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    return JobActionResponse(success=job.status == "success", message=f"日报生成{'成功' if job.status == 'success' else '失败'}: {job.error_message or ''}", job_id=job.id)
...
def trigger_send(db: Session = Depends(get_db), _=Depends(get_current_user)):
    ...
    return JobActionResponse(success=result.get("success", False), message=f"发送{'成功' if result.get('success') else '失败'}: {result.get('error', '')}")
